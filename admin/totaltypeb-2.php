...
try {
  $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8", $username, $password);
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $stmt = $conn->prepare("select form_1_2558.*, GROUP_CONCAT(ap1.valuekoon ORDER by ap1.type) as ap1,
  ...
  from form_1_2558, ap1, ap2, ap3, ap4, ap5, ap6, ap7, ap8, ap9, ap10, ap11, ap12, ap13, ap14, ap15, ap16, ap17, ap18, ap19, ap20, ap21, ap22, ap23
    ...
    <td style="text-align:center" class="cl<?php echo $r?>"><?php echo $vk[2]?></td> 
      ...
      <td></td>
        ...
        foreach($result as $row){
          ...
          ?>
        <td colspan="2"></td>
        <td style="text-align:center"><strong><font color="black"><?php echo $vk[0]?></strong></td>
        <?php
        }
        ?>
        </tr><?php
      ...
      <td></td>
        ...
        <?php
        //$stmt = $conn->prepare("SELECT sum(koon) FROM form_$id where kor = $i; select * from total_score"); 
        $stmt = $conn->prepare("SELECT GROUP_CONCAT(total_score.mp$i ORDER by type) as ap$i FROM total_score
        WHERE total_score.time = $y and ep = $ep GROUP by apid ORDER by apid"); 
        $stmt->execute();
        $result = $stmt->FetchAll(PDO::FETCH_ASSOC);
        foreach($result as $row){
          $vk = explode(",", $row["ap$i"]);
